...
namespace android {
    ...
    CAMERA_HAL_ERR_RET CameraHal :: GetCameraBaseParam(CameraParameters *pParam)
    {
        ...
        if(strstr(mCameraSensorName, "adv") != NULL){
            mPreviewFormat = V4L2_PIX_FMT_UYVY;
        }
        ...
        if(strstr(mCameraSensorName, "adv") != NULL){
            ...
            strcpy(supportedFPS, "25,30");
            pParam->set(CameraParameters::KEY_SUPPORTED_PREVIEW_FRAME_RATES, supportedFPS);
            pParam->set(CameraParameters::KEY_SUPPORTED_PREVIEW_FPS_RANGE, "(1000,25000),(5000,30000)");

            if (mCaptureDevice->DevGetStd(&TvType) <0){
                CAMERA_HAL_LOG_RUNTIME("get std error");
            }
            else{
                if(TvType == TV_PAL){
                    pParam->setPreviewSize(720, 576);
                    pParam->setPictureSize(720, 576);
                }else{
                    pParam->setPreviewSize(720, 480);
                    pParam->setPictureSize(720, 480);
                }
            }
        }
    ...
    status_t CameraHal::setOverlay(const sp<Overlay> &overlay)
    {
        ...
        if (overlay == NULL){
            ...
            if(strstr(mCameraSensorName, "adv") != NULL){
                overlay->setParameter(OVERLAY_SURFACE, OVERLAY_INTERLACED_SURFACE);
            }
        }
...
// These properties shall be defined by the device init.rc file
// according to its hw capabilities
#define HW_CAMERA_NUMBER_OF_CAMERAS 	"hw.camera.numberofcameras"
#define HW_CAMERA_TYPE_FMT				"hw.camera.camera%d.type"
#define HW_CAMERA_ORIENT_FMT			"hw.camera.camera%d.orient"
#define HW_CAMERA_FACING_FMT			"hw.camera.camera%d.facing"
#define HW_CAMERA_NAME_FMT				"hw.camera.camera%d.name"
#define HW_CAMERA_SRC_FMT				"hw.camera.camera%d.src"

static void GetCameraProperties(int cameraId, CameraInfo *cameraInfo)
{
	char orientStr[PROPERTY_VALUE_MAX];
	char typeStr[PROPERTY_VALUE_MAX];
	char nameStr[PROPERTY_VALUE_MAX];
	char facingStr[PROPERTY_VALUE_MAX];
	char propStr[30];

	memset(propStr, 0x00, sizeof(propStr));
	snprintf(propStr, sizeof(propStr), HW_CAMERA_TYPE_FMT, cameraId);
	property_get(propStr, typeStr, "usb");

	if(strstr(typeStr, "usb") != NULL)
		cameraInfo->cameraType = CAMERA_TYPE_USB;
	else if(strstr(typeStr, "embedded") != NULL)
		cameraInfo->cameraType = CAMERA_TYPE_EMBEDDED;
	else if(strstr(typeStr, "analog") != NULL)
		cameraInfo->cameraType = CAMERA_TYPE_ANALOG;
	else if(strstr(typeStr, "avin") != NULL)
		cameraInfo->cameraType = CAMERA_TYPE_AVIN;
	else if(strstr(typeStr, "digitaltv") != NULL)
		cameraInfo->cameraType = CAMERA_TYPE_DIGITALTV;
	else
		cameraInfo->cameraType = CAMERA_TYPE_USB;

	memset(propStr, 0x00, sizeof(propStr));
	snprintf(propStr, sizeof(propStr), HW_CAMERA_ORIENT_FMT, cameraId);
	property_get(propStr, orientStr, "0");
	cameraInfo->orientation = atoi(orientStr);

	memset(propStr, 0x00, sizeof(propStr));
	snprintf(propStr, sizeof(propStr), HW_CAMERA_FACING_FMT, cameraId);
	property_get(propStr, facingStr, "back");
	if(strstr(facingStr, "back") != NULL)
		cameraInfo->facing = CAMERA_FACING_BACK;
	else if(strstr(facingStr, "front") != NULL)
		cameraInfo->facing = CAMERA_FACING_FRONT;
	else
		cameraInfo->facing = CAMERA_FACING_BACK;

	memset(propStr, 0x00, sizeof(propStr));
	snprintf(propStr, sizeof(propStr), HW_CAMERA_NAME_FMT, cameraId);
	property_get(propStr, nameStr, "uvc");
	if(strstr(nameStr, "adv") != NULL)
		cameraInfo->sensor_type = CAMERA_SENSOR_TVIN;
	else
		cameraInfo->sensor_type = CAMERA_SENSOR_CAMERA;

	CAMERA_HAL_LOG_INFO("CameraId %d is %s: orient=%s, facing=%s, type=%s", cameraId, nameStr, orientStr, facingStr, typeStr);
}

int HAL_getNumberOfCameras()
{

	char camerasStr[PROPERTY_VALUE_MAX];
	property_get(HW_CAMERA_NUMBER_OF_CAMERAS, camerasStr, "0");
	return atoi(camerasStr);

}

void HAL_getCameraInfo(int cameraId, struct CameraInfo* cameraInfo)
{
	GetCameraProperties(cameraId, cameraInfo);
}
...
sp<CameraHardwareInterface> HAL_openCameraHardware(int cameraId) {
	...
	char propStr[30];
	char srcStr[PROPERTY_VALUE_MAX];
	char nameStr[PROPERTY_VALUE_MAX];
	int cameras = 0;
	int src;

	cameras = HAL_getNumberOfCameras();
	if (cameraId >= cameras)
	{
		CAMERA_HAL_ERR("Invalid cameraId: %d. Should be less than %d!", cameraId, cameras);
		return NULL;
	}
	...
	memset(propStr, 0x00, sizeof(propStr));
	snprintf(propStr, sizeof(propStr), HW_CAMERA_SRC_FMT, cameraId);
	property_get(propStr, srcStr, "0");
	src = atoi(srcStr);

	pCaptureDevice = createCaptureDevice(nameStr);
	...
	if (strstr(nameStr, "ov") != NULL) {
		pCameraHal->setPreviewRotate(CAMERA_PREVIEW_BACK_REF);
	}

	if(strstr(nameStr, "adv") != NULL) {
		if(pCaptureDevice->DevSetInputSource(src) < 0) {
			CAMERA_HAL_ERR("Error while setting input source to %d!", src);
			return NULL;
		}
	}

...
namespace android{
    V4l2CsiDevice :: V4l2CsiDevice(){
        ...
        mSupportedFmt[3] = v4l2_fourcc('U','Y','V','Y');
    ...
    CAPTURE_DEVICE_ERR_RET V4l2CsiDevice :: V4l2SetConfig(struct capture_config_t *pCapcfg)
    {
        ...
        if (strstr(mInitalDeviceName, ADV7180_NAME_STR) != 0)
        {
            v4l2_std_id std;

            if (ioctl(mCameraDevice, VIDIOC_G_STD, &std) < 0)
            {
                CAMERA_HAL_ERR("%s:%d  VIDIOC_G_STD failed\n", __FUNCTION__,__LINE__);
                return CAPTURE_DEVICE_ERR_SYS_CALL;
            }
            switch(std)
            {
                case V4L2_STD_NTSC:
                    CAMERA_HAL_LOG_INFO("V4l2SetConfig: V4L2_STD_NTSC");
                    break;
                case V4L2_STD_PAL:
                    CAMERA_HAL_LOG_INFO("V4l2SetConfig: V4L2_STD_PAL");
                    break;
            }

            if (ioctl(mCameraDevice, VIDIOC_S_STD, &std) < 0)
            {
                CAMERA_HAL_ERR("%s:%d  VIDIOC_S_STD failed\n", __FUNCTION__,__LINE__);
                return CAPTURE_DEVICE_ERR_SYS_CALL;
            }
        }
        ...
        if ((pCapcfg->fmt == V4L2_PIX_FMT_YUYV) || (pCapcfg->fmt == V4L2_PIX_FMT_UYVY))
	...
	CAMERA_HAL_LOG_RUNTIME("V4l2CsiDevice::V4l2SetConfig: Set the Format :%c%c%c%c\n",
    ...
    CAPTURE_DEVICE_ERR_RET V4l2CsiDevice :: V4l2GetStd(TVinType *pTvType){
        CAMERA_HAL_LOG_FUNC;
        if (mCameraDevice <= 0 || pTvType == NULL){
            return CAPTURE_DEVICE_ERR_BAD_PARAM;
        }
    ...
    CAPTURE_DEVICE_ERR_RET V4l2CsiDevice :: V4l2SetInputSrc(int src){
		CAMERA_HAL_LOG_FUNC;
		if (mCameraDevice <= 0){
			return CAPTURE_DEVICE_ERR_BAD_PARAM;
		}
		...
		ctrl.id = V4L2_CID_PRIVATE_BASE + 4;
		ctrl.value = src;
		if (ioctl(mCameraDevice, VIDIOC_S_CTRL, &ctrl) < 0)
		{
			CAMERA_HAL_ERR("set input src failed\n");
			return CAPTURE_DEVICE_ERR_SYS_CALL;
		}
	...
	}else if (strstr(mInitalDeviceName, ADV7180_NAME_STR) != 0){
		pic_waite_buf_num = 1;
		if (capturewidth == 720 && captureheight == 480)
			capturemode = 0;	/* NTSC mode */
		else if (capturewidth == 720 && captureheight == 576)
			capturemode = 1;	/* PAL mode */
		else{
			CAMERA_HAL_ERR("The camera mode is not supported!!!!");
			return CAPTURE_DEVICE_ERR_BAD_PARAM;
		}
    ...
    CAPTURE_DEVICE_ERR_RET V4l2CsiDevice :: V4l2SetRot(struct capture_config_t *pCapcfg){
        ...
        if (strstr(mInitalDeviceName, ADV7180_NAME_STR) != 0)
        {
            //TVin chip doesn't support rotation
            return ret;
        }

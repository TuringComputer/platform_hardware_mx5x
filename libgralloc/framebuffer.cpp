...
static int set_graphics_fb_mode(int fb, int dual_disp)
{
    ...
    // NOTE - ADENEO:
    // Keep the setting from the command-line.
    // Not returning here causes Android to use 1080p mode when XGA is
    // requested.
    if(fb==0 && strstr(cmd_line, "ldb")) {
        LOGI("HACK - gralloc: LDB detected, not autosetting the resolution");
        return 0;
    }

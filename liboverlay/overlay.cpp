...
int overlay_init_v4l(struct overlay_control_context_t *dev, int layer)
{
        ...
        for(int buf_index=0;buf_index < dev->v4l_bufcount;buf_index++) {
            ...
            OVERLAY_LOG_INFO("v4l buf[%d] 0x%p: vir 0x%p,phy 0x%x, size %d", buf_index,v4lbuf,vir_addr,v4lbuf->m.offset,v4lbuf->length);
            v4lbuf++;
        }
        ...
        overlay_handle_t handle)
{
    ...
    ctx->data_shared->surface_mode = OVERLAY_NORMAL_SURFACE;
    ...
    switch(param) {
        ...
        case OVERLAY_BUFNUM:
            ...
            case OVERLAY_SURFACE:
                data_shared->surface_mode = value;
            break;
        default:
            break;
    }

                                                            ...
                                                            int original_rotation)
{
    ...
    RECTTYPE orig_win = *original_win;
    RECTTYPE *pOrigWin = &orig_win;
    if(original_rotation >= 4){
        int temp = pOrigWin->nHeight;
        pOrigWin->nHeight = pOrigWin->nWidth;
        pOrigWin->nWidth = temp;
    }

    if(pOrigWin->nWidth * new_border->nHeight >= pOrigWin->nHeight * new_border->nWidth) {
        /* Crop display height */
        nHeight = pOrigWin->nHeight * new_border->nWidth / pOrigWin->nWidth;
        ...
        /* Crop display width */
        nWidth = pOrigWin->nWidth * new_border->nHeight / pOrigWin->nHeight;
...
//Make a switch to TVOUT FB1
bool OverlayThread::switchTvOut(struct overlay_control_context_t * dev)
{
    OVERLAY_LOG_FUNC;

    if(dev->display_mode == DISPLAY_MODE_TV) { 
        OVERLAY_LOG_WARN("Already in TVOUT mode, ignore this switch");
        return false;
    }
    ...
    OVERLAY_LOG_INFO("get property %d/%d/%d/%d", dev->user_property.nLeft, dev->user_property.nTop, dev->user_property.nWidth, dev->user_property.nHeight);

    //Auto set the TV FB MODE
    autoSetTVMode(1, 1);

    //set the output format to RGBP since it may be mapped as rgbp in libgralloc
    //for dual UI feature. Need a fix in kernel to make the new format
    //setting valid in v4l tv output layer if FB1 already been set to another
    //format
    dev->outpixelformat = v4l2_fourcc('R', 'G', 'B', 'P');
    ...
    if(overlay_init_output(dev, OVERLAY_FB_TVOUT)<0){
        OVERLAY_LOG_ERR("Error!init output FB failed");
        return false;
    }
    dev->display_mode = DISPLAY_MODE_TV;
    return true;
}

bool OverlayThread::checkSecVideoParam(struct overlay_control_context_t *dev)
{
    bool retValue = false;
    if((sec_video_mIPUInputParam.width != dev->overlay_intances[0]->mHandle.width) ||
      (sec_video_mIPUInputParam.height != dev->overlay_intances[0]->mHandle.height) ||
      (sec_video_mIPUInputParam.input_crop_win.pos.x != dev->overlay_intances[0]->mDataShared->crop_x) ||
      (sec_video_mIPUInputParam.input_crop_win.pos.y != dev->overlay_intances[0]->mDataShared->crop_y) ||
      (sec_video_mIPUInputParam.input_crop_win.win_w != dev->overlay_intances[0]->mDataShared->crop_w) ||
      (sec_video_mIPUInputParam.input_crop_win.win_h != dev->overlay_intances[0]->mDataShared->crop_h) ||
      (sec_video_mIPUInputParam.fmt != get_v4l2_fmt(dev->overlay_intances[0]->mHandle.format))) {
        ...
        sec_video_mIPUInputParam.width = dev->overlay_intances[0]->mHandle.width;
        sec_video_mIPUInputParam.height = dev->overlay_intances[0]->mHandle.height;
        sec_video_mIPUInputParam.input_crop_win.pos.x = dev->overlay_intances[0]->mDataShared->crop_x;
        sec_video_mIPUInputParam.input_crop_win.pos.y = dev->overlay_intances[0]->mDataShared->crop_y;
        sec_video_mIPUInputParam.input_crop_win.win_w = dev->overlay_intances[0]->mDataShared->crop_w;
        ...
        else if(m_dev->overlay_intances[0]->mHandle.format == HAL_PIXEL_FORMAT_CbYCrY_422_I) {
            sec_video_mIPUInputParam.fmt = v4l2_fourcc('U', 'Y', 'V', 'Y');
        }
        ...
        sec_video_mIPUInputParam.user_def_paddr[0] = dev->overlay_intances[0]->mDataShared->queued_bufs[m_dev->overlay_intances[0]->mDataShared->queued_head];
        sec_video_mIPUOutputParam.fmt = v4l2_fourcc('U', 'Y', 'V', 'Y');
        /* should set the fmt based on the fb1's setting
        switch (fb_var.bits_per_pixel) {
            case 24:
                sec_video_mIPUOutputParam.fmt = IPU_PIX_FMT_BGR24;
                OVERLAY_LOG_WARN("output fmt BGR24");
            break;
            case 32:
                sec_video_mIPUOutputParam.fmt = IPU_PIX_FMT_BGR32;
                OVERLAY_LOG_WARN("output fmt BGR32");
            break;
            case 16:
                sec_video_mIPUOutputParam.fmt = IPU_PIX_FMT_RGB565;
                OVERLAY_LOG_WARN("output fmt RGB565");
            break;
        }
        */
...
bool OverlayThread::switchDualVideoSinUI(struct overlay_control_context_t * dev)
{
    int fb_dev;
    ...
    if(overlay_init_output(dev, OVERLAY_FB_NORMAL)<0){
...
bool OverlayThread::switchNormalOverlay(struct overlay_control_context_t * dev)
{
    #ifdef OVERLAY_V4L_OUT 
    if(dev->stream_on){
        int type = V4L2_BUF_TYPE_VIDEO_OUTPUT;
...
bool OverlayThread::switchSinVideoDualUI(struct overlay_control_context_t * dev)
{
    OVERLAY_LOG_FUNC;
    #ifdef OVERLAY_V4L_OUT
    if(dev->stream_on){
    ...
    OVERLAY_LOG_INFO(" FB1 resolution %d,%d", dev->fb1_resolution.nWidth, dev->fb1_resolution.nHeight);

    // use fb1 resolution to initialize v4l
    dev->xres = dev->fb1_resolution.nWidth;
    dev->yres = dev->fb1_resolution.nHeight;

    // use the default v4l layer since overlay has been switch to fb1
    //if(overlay_init_v4l(dev, DEFAULT_V4L_LAYER)<0){
    if(overlay_init_output(dev, OVERLAY_FB_NORMAL)<0){
       OVERLAY_LOG_ERR("Error!init output failed");
       return false;
...
bool OverlayThread::checkDispMode(struct overlay_control_context_t * dev)
{
    char value[PROPERTY_VALUE_MAX];
    bool bTVOUTDisplay = false;
    bool bSecDisplay = false;

    OVERLAY_LOG_FUNC;

    //sys.VIDEO_TVOUT_DISPLAY should be set only when sys.SECOND_DISPLAY_ENABLED
    //has been set
    property_get("sys.VIDEO_TVOUT_DISPLAY", value, "");
    if (strcmp(value, "1") == 0) {
        bTVOUTDisplay = true;
    }
    ...
    //switch to TVOUT mode(video only to fb1)
    if((dev->display_mode != DISPLAY_MODE_TV)&& bTVOUTDisplay && bSecDisplay){
        OVERLAY_LOG_INFO("switch to TV output mode");
        switchTvOut(dev);
        return true;
    }

    //Switch to normal mode(video only to fb2, overlayed with fb0)
    if((dev->display_mode != DISPLAY_MODE_NORMAL)&& !bSecDisplay){
       OVERLAY_LOG_INFO("switch to NORMAL output mode");
       switchNormalOverlay(dev);
       return true;
    }

    if((dev->display_mode == DISPLAY_MODE_NORMAL)&& bSecDisplay){
        dev->display_mode = DISPLAY_MODE_DUAL_DISP;

        if(dev->video_play_mode == SIN_VIDEO_DUAL_UI){
            OVERLAY_LOG_INFO("switch to SIN_VIDEO_DUAL_UI output mode");
            switchSinVideoDualUI(dev);
        }
	    else{
            //Dual video case
            //Render the overlay to FB2(Overlay with FB0) and FB1
            OVERLAY_LOG_INFO("switch to DUAL_VIDEO_SIN_UI output mode");
            switchDualVideoSinUI(dev);
        }
        return true;
    }

    //Check whether dual video case have output or input change for sec_video_mIPUHandle
    if((dev->display_mode == DISPLAY_MODE_DUAL_DISP) && (dev->video_play_mode != SIN_VIDEO_DUAL_UI)){
        checkSecVideoParam(dev);
    }
                ...
                if (dev->video_play_mode == DUAL_VIDEO_SIN_UI) {
                   ...
                   *rotation = overlayObj->rotation;
                   ...
                   if(dev->overlay_number == 1 && overlayObj->mDataShared->for_playback){
                       ...
                       calcDispWinWithAR(&original_win, &new_border, new_win, overlayObj->rotation);
                   }
                   else{
...
bool OverlayThread::threadLoop() {
        ...
        int overlay0_interlaced;
        int overlay1_interlaced;
        ...
        while(m_dev&&(m_dev->overlay_running)) {
            ...
            if(overlayObj0 || overlayObj1){
                ...
                if(current_time.tv_sec > last_check_time.tv_sec){
                    modeChange = checkDispMode(m_dev);
                    if(modeChange){
                        modeChange0 = true;
                        modeChange1 = true;
                        modeChange = false;
            ...
            if(overlayObj0) {
                ...
                if((dataShared0->in_destroy)||(!overlayObj0->visible && (m_dev->video_frames > 0 ))) {
                ...
                overlay0_interlaced = dataShared0->surface_mode;
            ...
            if(overlayObj1) {
                ...
                //If this overlay is under destroy, just return all buffers in queue
                //The user should guarantee the buffer all to be flushed before destroy it.
                if(dataShared1->in_destroy||(!overlayObj1->visible && (m_dev->video_frames > 0 ))) {
                     OVERLAY_LOG_INFO("Overlay in destroy or invisible,flush queue as queued_count %d,queued_head %d",
                ...
                overlay1_interlaced = dataShared1->surface_mode;
            ...
            //to v4l current buffer
            unsigned int outPhyAddr = 0;
            #ifdef OVERLAY_V4L_OUT
            //Dequeue a V4L2 Buffer
            struct v4l2_buffer v4lbuf;
            memset(&v4lbuf, 0, sizeof(v4l2_buffer));
            struct v4l2_buffer *pV4LBuf = &v4lbuf;

            if(v4l2_device_fetch(m_dev, pV4LBuf) < 0){
                OVERLAY_LOG_ERR("Error!Cannot fetch a v4l buffer for output!");
                //reset latest queued buffer, since all buffer will start as init
                mLatestOutBuf = 0;
                goto free_buf_exit;
            }

            outPhyAddr = pV4LBuf->m.offset;
            ...
            #else
            //Fetch output fb buffer for mixing
            void *pOutBuf = NULL;
            if(output_device_fetch(m_dev, &pOutBuf, &outPhyAddr) < 0){
                OVERLAY_LOG_ERR("Error!Cannot fetch a output buffer for output!");
                mLatestOutBuf = 0;
                goto free_buf_exit;
            }
            OVERLAY_LOG_RUNTIME("Fetch output buffer: virtual %x,physical %x",
                    pOutBuf, outPhyAddr);
            #endif
            ...
            {	//record the displayed mask
                ...
                int bufLen;
                char * frame;
                #ifndef OVERLAY_V4L_OUT
                bufIndex = m_dev->output_next_buf;
                frame = (char *)pOutBuf;
                bufLen = m_dev->output_frame_size;
                #else
                bufIndex = pV4LBuf->index;
                frame = m_dev->v4lbuf_addr[pV4LBuf->index];
                bufLen = pV4LBuf->length;
                #endif
                unsigned int CurBufMask = 1 << (bufIndex);
                DisplayBufMask |= CurBufMask;
                if(DisplayChanged == 1){
                    if ((ClearBufMask & CurBufMask) != 0){
                        fill_frame_back(frame, bufLen, m_dev->xres, m_dev->yres, m_dev->outpixelformat);
               ...
               (dataShared0->buf_showed)*/)
            {
                ...
                if((mLatestOutBuf)&&
                   (mLatestOutBuf != outPhyAddr)) {
                    ...
                    mIPUInputParam.user_def_paddr[0] = mLatestOutBuf;
                    ...
                    mIPUOutputParam.user_def_paddr[0] = outPhyAddr;
                    ...
                    OVERLAY_LOG_RUNTIME("Copyback(before) Input param: width %d, height %d, fmt %d, crop_win pos x %d, crop_win pos y %d, crop_win win_w %d,crop_win win_h %d",
                     ...
                     do {
                        if(overlay0_interlaced == OVERLAY_INTERLACED_SURFACE){
                            mIPUInputParam.motion_sel = HIGH_MOTION;
                            mIPURet =  mxc_ipu_lib_task_init(&mIPUInputParam,NULL,&mIPUOutputParam,
                                    OP_NORMAL_MODE|TASK_VDI_VF_MODE,&mIPUHandle);
                        }else{
                            mIPURet =  mxc_ipu_lib_task_init(&mIPUInputParam,NULL,&mIPUOutputParam,
                                    OP_NORMAL_MODE|TASK_PP_MODE,&mIPUHandle);
                        }
                        //ipu limitation on split mode
                        if(mIPURet == IPU_STATE_SPLIT_MODE_WIDTH_OVER){
                            //reduce the width by 8 pixels
                            mIPUOutputParam.output_win.pos.x += 4;
                            mIPUOutputParam.output_win.win_w -= 4;
                        }
                        else if(mIPURet == IPU_STATE_SPLIT_MODE_HEIGHT_OVER){
                            //reduce the height by 8 pixels
                            mIPUOutputParam.output_win.pos.y += 4;
                            mIPUOutputParam.output_win.win_h -= 4;
                        }
    
                    }
                    while((mIPURet == IPU_STATE_SPLIT_MODE_WIDTH_OVER)||
                          (mIPURet == IPU_STATE_SPLIT_MODE_HEIGHT_OVER));
                    ...
                    mIPURet = mxc_ipu_lib_task_buf_update(&mIPUHandle, mLatestOutBuf, outPhyAddr,NULL,NULL,NULL);
                    if (mIPURet < 0) {
            ...
            if(overlay_buf0 && (overlayObj0->outX != 0  ||
                ...
                else if(overlayObj0->mHandle.format == HAL_PIXEL_FORMAT_CbYCrY_422_I) {
                    mIPUInputParam.fmt = v4l2_fourcc('U', 'Y', 'V', 'Y');
                }
                ...
                mIPURet = mxc_ipu_lib_task_buf_update(&mIPUHandle, overlay_buf0, outPhyAddr, NULL,NULL,NULL);
                if (mIPURet < 0) {
            ...
            if(overlay_buf1 && (overlayObj1->outX != 0  ||
                ...
                else if(overlayObj1->mHandle.format == HAL_PIXEL_FORMAT_CbYCrY_422_I) {
                    mIPUInputParam.fmt = v4l2_fourcc('U', 'Y', 'V', 'Y');
                }
                else if(overlayObj0->mHandle.format == HAL_PIXEL_FORMAT_YCbCr_422_I) {
                    mIPUInputParam.fmt = v4l2_fourcc('4', '2', '2', 'P');
                ...
                OVERLAY_LOG_RUNTIME("Obj1 Input param:width %d,height %d,fmt %d,crop_win pos x %d,crop_win pos y %d,crop_win win_w %d,crop_win win_h %d",
                                 ...
                                 mIPUInputParam.input_crop_win.win_h);     
                do {
                    if(overlay1_interlaced == OVERLAY_INTERLACED_SURFACE){
                        mIPUInputParam.motion_sel = HIGH_MOTION;
                        mIPURet =  mxc_ipu_lib_task_init(&mIPUInputParam,NULL,&mIPUOutputParam,
                                OP_NORMAL_MODE|TASK_VDI_VF_MODE,&mIPUHandle);
                    }else{
                ...
                mIPURet = mxc_ipu_lib_task_buf_update(&mIPUHandle,overlay_buf1,outPhyAddr,NULL,NULL,NULL);
                if (mIPURet < 0) {
...
queue_buf_exit:
            //Queue the mixed V4L2 Buffer for display. Only when overlay is visible, queue the second buffer
	    if (((m_dev->video_frames == 1) && ((overlayObj0 ? overlayObj0->visible : 0) 
		|| (overlayObj1 ? overlayObj1->visible : 0))) || (m_dev->video_frames != 1))
	    {
            #ifdef OVERLAY_V4L_OUT
            if(v4l2_device_post(m_dev, pV4LBuf) < 0){
                OVERLAY_LOG_ERR("Error!Cannot post a buffer to v4l");
                //reset latest queued buffer, since all buffer will start as init
                mLatestOutBuf = 0;
                goto free_buf_exit;
            }
            #else
            if(output_device_post(m_dev) < 0){
                OVERLAY_LOG_ERR("Error!Cannot post a buffer to output device");
                //reset latest queued buffer, since all buffer will start as init
                mLatestOutBuf = 0;
                goto free_buf_exit;
